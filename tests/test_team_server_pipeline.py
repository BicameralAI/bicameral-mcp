...
async def test_pipeline_short_circuits_on_negative_classification():
    ...
    result = await extract_decision_pipeline(
        text="random chatter", message={"text": "random chatter"},
        context={}, rules_or_disabled=rules, llm_extract_fn=stub_llm,
    )
    assert calls["n"] == 0
...
async def test_slack_worker_routes_through_pipeline_with_thread_context(monkeypatch):
    ...
    _os.environ["BICAMERAL_TEAM_SERVER_SECRET_KEY"] = (
        "EYSr77qKo0UijHGnER5qYFBY5ZZePeWeE-ZMWYXyKKA="
    )
    from team_server.config import TeamServerConfig, SlackConfig, SlackHeuristics
    from team_server.config import HeuristicGlobalRules
    ...
    async def stub_pipeline(*, text, message, context, rules_or_disabled, llm_extract_fn):
        ...
        return {
            "decisions": [], "classifier_version": "h-test",
            "matched_triggers": [], "extractor_version": None, "skipped": False,
        }
    ...
    try:
        ...
        await poll_once(
            db_client=client, slack_client=_SlackStub(),
            workspace_team_id="T1", channels=["C1"],
            extractor=stub_extractor, config=config,
        )
...
async def test_notion_worker_routes_through_pipeline_with_edit_context(monkeypatch):
    ...
    import os as _os
    _os.environ["BICAMERAL_TEAM_SERVER_SURREAL_URL"] = "memory://"
    from team_server.config import TeamServerConfig, NotionConfig, NotionHeuristics
    from team_server.config import HeuristicGlobalRules
    from team_server.db import build_client
    from team_server.schema import ensure_schema
    ...
    config = TeamServerConfig(
        notion=NotionConfig(heuristics=NotionHeuristics(
...
async def test_pipeline_skips_when_rules_disabled():
    ...
    result = await extract_decision_pipeline(
        text="anything", message={"text": "anything"},
        context={}, rules_or_disabled=RulesDisabled(),
