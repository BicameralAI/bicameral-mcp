...
def test_session_end_command_uses_hyphen_slash_command():
    """Regression guard for issue #177: the slash command spawned by the
    SessionEnd hook must use /bicameral-capture-corrections (folder-name
    match), not the broken plugin-namespace form /bicameral:capture-corrections.

    Post Priority-B-v0 refactor, the hook command itself dispatches to
    `python3 -m events.session_end_bridge`; the slash command lives inside
    the bridge module's CHILD_CLAUDE_CMD. The regression is checked at
    that boundary instead."""
    from events import session_end_bridge
    ...
    child_cmd_str = " ".join(session_end_bridge.CHILD_CLAUDE_CMD)
    assert "/bicameral-capture-corrections" in child_cmd_str
    assert "/bicameral:capture-corrections" not in child_cmd_str
