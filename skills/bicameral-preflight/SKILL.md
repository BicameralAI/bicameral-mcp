...
code-locator graph (callers / callees / imports / inheritance), so a
decision bound to `app/src/lib/git/reorder.ts` still surfaces when the
caller passes the structurally-near `app/src/ui/multi-commit-operation/reorder.tsx`.
You should still pass concrete paths discovered in step 1 — the expansion
lifts the recall ceiling on near-misses, it doesn't replace caller-side
discovery. Decisions reached only via the expansion carry `confidence=0.7`
in the response (vs `0.9` for direct pins), and `sources_chained`
includes `"region_graph_expanded"` when expansion contributed at least
one hit. Caller can de-prioritize expanded matches without losing them.
