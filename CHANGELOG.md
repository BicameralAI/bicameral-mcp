...
- `setup_wizard._build_session_end_command` now renders OS-specific shape via new `platform: str | None = None` argument defaulting to `sys.platform` (#200 Phase 1, A1). POSIX keeps bash conditional shape; Windows gets a cmd.exe shape (`if exist .bicameral if not defined ... (set ... && python ...)`). Both shapes invoke `python` (not `python3` — Windows installers don't symlink `python3`; modern Linux distros symlink `python` → `python3`). Closes the SessionEnd hook silent-failure on Windows MinGW. New `_session_end_command_for_platform` helper. 4 new behavioral tests; existing drift tests updated to be platform-aware. Capture-corrections SKILL.md `python3 scripts/hooks/transcript_archive.py` references switched to `python` to match. A7 telemetry transparency notes added above the SessionEnd batch consent prompt in capture-corrections and the Step 3.5 consent prompt in report-bug (instruction-level — explicitly tracked as suggestive, not governance, per #205 doctrine).
