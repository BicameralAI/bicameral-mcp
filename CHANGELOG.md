...
- **#58 Phase A — M6 preflight retrieval recall eval (warn-only).** Sibling of the M2 grounding-recall eval (#284): synthetic-fixture benchmark that drives `handle_preflight` against a fresh memory:// ledger seeded per case, classifies outcome (`surfaced` / `missed` / `error`), aggregates per miss-mode. 25 cases at `tests/fixtures/preflight_m6/dataset.py` balanced across the three miss modes documented in the issue body — `vocabulary_mismatch` (8), `unbound_decision` (8), `transitive_relevance` (9). Three deliberately-split measurement axes: overall recall (gate ≥ 0.70 = wiki M6 threshold), per-miss-mode recall (gate ≥ 0.50; picks Phase B direction), fire rate (gate ≥ 0.60; secondary diagnostic). Seeder at `tests/eval/_preflight_m6_seeder.py` uses real ingest + bind path so each case's row has production shape (status, signoff, binds_to). Runner at `tests/eval_preflight_m6_recall.py` + markdown step-summary at `tests/eval_preflight_m6_summary.py` (per-miss-mode table + collapsible missed-case detail). 16 sociable unit tests in `tests/test_preflight_m6_eval.py` (real `M6Case` dataclasses + `SimpleNamespace` responses per CLAUDE.md sociable-testing rule, no `MagicMock`). New CI step warn-only initially — Phase A is **measurement only** (recall regression risk = zero, no runtime change). Phase B (the actual optimization — multi-hop expansion / semantic search / LLM reranker) gated on this PR's first stable baseline. Plan: `plan-58-preflight-decision-detection.md`. Signoff: https://github.com/BicameralAI/bicameral-mcp/issues/58#issuecomment-4416852574.
...
- **M2 failure-mode enumeration on the GitHub step summary (#280, Jin's PR-#288 followup).** Per Jin's note that "behavioral decisions likely cannot easily be grounded to code … failure scenarios are useful for cross-functional design discussion": `tests/eval_grounding_recall.py` now classifies each case into a PM-readable category (`wrong_module` / `wrong_intent` / `cross_language_confusion` / `wrong_symbol_in_right_file` / `hallucinated_symbol` / `span_mismatch` / `aborted_correctly` / `aborted_incorrectly` / `eval_error`) via a deterministic post-hoc classifier. Each category carries a documented PM-actionable next step (e.g. `cross_language_confusion` → "make decision text mention runtime explicitly"; `aborted_correctly` → "behavioral decisions correctly route to PM review"). The renderer adds a new "Failure modes (top categories — PM-actionable)" section to the step summary, ranked by miss count with up to 2 example cases per category (case_id + agent reasoning excerpt). Aggregate metrics + per-case-type recall + raw miss list all kept — the new section is additive, surfaced above the per-case miss list. 13 table-driven unit tests in `tests/test_grounding_failure_mode.py` pin the classifier behavior across all 10 categories. Cache key unchanged → 0.913/0.913 baseline cache stays valid; no re-record needed.
...
- **M2 eval robustness — bounded retry on transient API failures + per-case `eval_error` outcome.** The hard-gate flip surfaced an existing flakiness: a single `httpx.ReadTimeout` on one of N tool-use turns crashed the whole eval run (the per-case catch only handled `RuntimeError`, not `httpx` exceptions). `_call_messages_api` in `tests/eval/_bind_judge.py` now retries 3× with exponential backoff (2s/8s/32s) on `httpx.TimeoutException`, `NetworkError`, `RemoteProtocolError`, plus `429` and `5xx` server-side responses. After all retries fail it raises `RuntimeError` with a bounded message; the runner's per-case catch now broadened to `except Exception` records that case as `eval_error` and continues with the rest. Aggregate gate is still applied — if so many cases erred that recall fell below 0.80, the gate fails. Renderer adds `eval_error` to the outcome-breakdown table and surfaces the error message inline in the missed-cases list.
...
- **Telemetry env-var namespace consolidated into `BICAMERAL_TELEMETRY` (#192).** Single env var now accepts `0`/`off`/`false`/`no` (all off), `1`/`on`/`true`/`yes` (relay only — preserves pre-#192 default), or a csv list of source names (`relay`, `preflight`, `raw`). New `telemetry_flags.py` module owns parsing; `consent.telemetry_allowed()` and `preflight_telemetry.{telemetry_enabled, raw_capture_enabled}` delegate to it. Legacy `BICAMERAL_PREFLIGHT_TELEMETRY=1` and `BICAMERAL_PREFLIGHT_TELEMETRY_RAW=1` continue to work via deprecation shim — first read of either emits a one-line stderr warning per process. Removed in v1.x. CSV form is explicit: what's listed is on, what's not is off (so `BICAMERAL_TELEMETRY=preflight,raw` turns OFF the default-on relay — documented in the setup wizard). `raw` always implies `preflight`. **Backwards-compat preservation**: any non-OFF value that doesn't match the canonical bool/csv shape (e.g. `BICAMERAL_TELEMETRY=enabled` or other custom truthy strings used in pre-#192 deployments) maps to relay-only with a stderr warning pointing at the canonical form, matching pre-#192 behavior. 35 fixtures in new `tests/test_telemetry_flags.py` cover all forms + integration with the existing call sites.
